...
class ItemWaitingCreateSession extends StatefulWidget {
  const ItemWaitingCreateSession({super.key});

  @override
  State<ItemWaitingCreateSession> createState() =>
      _ItemWaitingCreateSessionState();
}

class _ItemWaitingCreateSessionState extends State<ItemWaitingCreateSession> {
  ...
  _loadingResource() async {
    ...
    await PersonalItemService()
        .getItemWaittingCreateSession(UserProfile.user!.userId!)
        .then((value) {
      setState(() {
        bookingItems = value;
      });
